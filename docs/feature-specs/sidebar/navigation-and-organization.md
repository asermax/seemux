...
- Given a tab row, when hovered, then the cursor changes to a pointer hand
- Given a tab row, when PR number/URL are set, then PR label shows "PR#N" text in accent color; Ctrl+clicking opens the PR URL; Ctrl+hovering underlines the PR text; hovering shows the URL as a tooltip
